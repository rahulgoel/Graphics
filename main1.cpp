	...
	switch (key) {
		...
		case ' ':
			...
			for(int i = 0; i < 20; i++) {
				...
				ball->color = Vec3f( randomFloat() + 0.8f,
									 randomFloat() + 0.8f,
									 randomFloat() + 0.8f);
...
void drawScene() {
	...
	GLfloat lightPos[] = {1.0f, 0.2f, 0.0f, 1.0f};
	...
	for(unsigned int i = 0; i < _balls.size(); i++) {
		...
		glColor4f(ball->color[0], ball->color[1], ball->color[2],0.6);
		glutSolidSphere(ball->r, 500, 500); //Draw a sphere
		glPopMatrix();
	}
